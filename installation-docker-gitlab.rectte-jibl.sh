...
# 				 # l'adresse IP qui sera utilisée par les connecteurs HTTP/HTTPS de a seconde instance Gitlab
...
#                >>>   export NO_PORT_IP_SRV_GITLAB2=8880
# --------------------------------------------------------------------------------------------------------------------------------------------
...
# Retrait de l'option hostname du coker run: les configuration DNS en milieu hostile, seront revue dans une seconde passe d'évolution du provisionning
sudo docker run --detach --publish $ADRESSE_IP_SRV_GITLAB:433:443 --publish $ADRESSE_IP_SRV_GITLAB:$NO_PORT_IP_SRV_GITLAB:80 --publish $ADRESSE_IP_SRV_GITLAB:2227:22 --name $NOM_DU_CONTENEUR_CREE --restart always --volume $CONTENEUR_GITLAB_MAPPING_HOTE_CONFIG_DIR:$GITLAB_CONFIG_DIR --volume $CONTENEUR_GITLAB_MAPPING_HOTE_LOG_DIR:$GITLAB_LOG_DIR --volume $CONTENEUR_GITLAB_MAPPING_HOTE_DATA_DIR:$GITLAB_DATA_DIR $NOM_IMAGE_DOCKER_INSTANCES_GIROFLE
...
##########################################################################################
#											ANNEXE:								   	   	 #  
#			Configurations des noms de domaines pour l'accès à l'instance gitlab   	   	 #  
##########################################################################################
##########################################################################################
# ----------------------------------------------------------------------------------------
